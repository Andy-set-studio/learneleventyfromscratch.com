...
3. We then tell the feed reader (or whatever consumes this feed) when it was last updated. We do that with the `getNewestCollectionItemDate | dateToRfc3339` filters, provided by the RSS plugin by Eleventy. We pass the entire `blog` collection in there to do that. `getNewestCollectionItemDate` takes the blog collection and gets the most recent updated content date from the collection. Then it passes this date to `dateToRfc3339` which formats the date correctly for the XML entry.
