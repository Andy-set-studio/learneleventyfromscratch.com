...
We’re going to mitigate this risk by using an even better library than `node-fetch` to do this for us. It’s made by Eleventy and it’s called [@11ty/eleventy-fetch](https://github.com/11ty/eleventy-fetch) (Renamed from @11ty/eleventy-cache-assets).
...
That cleans out the fetch library we _were_ using and installs the custom fetch + cache library we’re _going_ to use instead.
